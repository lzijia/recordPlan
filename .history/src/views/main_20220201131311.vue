<template>
  <div>
    ...
    <van-popup v-model:show="showExportFilePopup"
               :style="{'width':'90%','height':'200px'}">
      <div>请自行复制数据并保存为txt格式的文件</div>
      <textarea>{{exportDataList}}</textarea>
    </van-popup>
  ...
  methods: {
    ...
    exportPlanData() {
      ...
      axios.get("./data/datetimeType.json").then((res) => {
        ...
        this.exportDataList = [];
        this.exportDataList.push("start:\r\n");
        this.exportDataList.push(planType);
        this.exportDataList.push(plan);
        this.exportDataList.push(planDetailContent);
