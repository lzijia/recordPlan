<template>
  <div>
    ...
    <van-popup v-model:show="showimportFilePopup"
      ...
      <div>请选择txt文件,文件内容可参考导出的数据格式</div>
  ...
  methods: {
    ...
    exportData(name: string, val: []) {
      if (window.navigator.msSaveBlob) {
        //ie下载
        window.navigator.msSaveBlob(new Blob(val), name);
      } else {
        const URL = window.URL || window.webkitURL;
        const url = URL.createObjectURL(new Blob(val));
        this.aDownlad(url, name);
      }
      ...
      //window.open(new Blob(val));
      //location.href = url;
      Toast("导出txt文件成功");
    },
    aDownlad(url: string, fileName: string) {
      const link = document.createElement("a");
      link.href = url;
      link.target = "_blank";
