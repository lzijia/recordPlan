    ...
    pushRemind() {
      ...
      if (window.Notification && Notification.permission !== "denied") {
        Notification.requestPermission(function (status) {
          if (status === "granted" && that.planContentList.length > 0) {
            let body = "";
            for (let i in that.planContentList) {
              if (that.planContentList[i].status === "1") {
                body =
                  body +
                  that.planContentList[i].planType +
                  "\n" +
                  that.planContentList[i].datetimeType +
                  " " +
                  that.planContentList[i].datetime +
                  "\n" +
                  that.planContentList[i].planContent;
              }
            }
