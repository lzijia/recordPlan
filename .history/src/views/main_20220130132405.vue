...
@Options({
  ...
  methods: {
    ...
    exportPlanData() {
      ...
      //获取计划
      let datetimeTypeContent = [];
      axios.get("./data/datetimeType.json").then((res) => {
        datetimeTypeContent = res.data;
        for (let datetimeTypeContentIndex in datetimeTypeContent) {
          alert(datetimeTypeContent[datetimeTypeContentIndex].en_name);
