...
@Options({
  ...
  methods: {
    ...
    remind() {
      ...
      for (let i in this.planContentList) {
        if (this.planContentList[i].status === "1") {
          let datetime = this.planContentList[i].datetime.split(":");
          let nowDate = new Date();
          let dateSplit = this.datetime.split("-");
          if (
            nowDate.getFullYear() == parseInt(dateSplit[0]) &&
            nowDate.getMonth() + 1 == parseInt(dateSplit[1]) &&
            nowDate.getDate() == parseInt(dateSplit[2]) &&
            parseInt(datetime[0]) >= nowDate.getHours() &&
            parseInt(datetime[1]) > nowDate.getMinutes()
          ) {
            let second =
              (parseInt(datetime[0]) - nowDate.getHours()) * 3600000 +
              (parseInt(datetime[1]) - nowDate.getMinutes()) * 60000;

            let secondPretime = second - 300000;
            if (secondPretime > 0) {
              second = secondPretime;
            }
            let watchTime = setTimeout(() => {
              if (window.Notification && Notification.permission !== "denied") {
                let that = this;
                Notification.requestPermission(function (status) {
                  if (status === "granted") {
                    window.plus.push.createMessage(
                      that.planContentListTitle[i]
                    );
                    // new Notification("计划记录管理", {
                    //   body: that.planContentListTitle[i],
                    // });
                  }
                });
              }
            }, second);
            this.timer.push(watchTime);
          }
        }
      }
    },
