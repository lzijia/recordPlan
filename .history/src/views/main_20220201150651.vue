<template>
  <div>
    ...
    <ul class="setList"
      ...
      <li @click="exportPlanData">查看计划数据</li>
    ...
    <van-popup v-model:show="showImportFilePopup"
      ...
      <div>请选择txt文件,文件内容可参考查看的数据格式</div>
    ...
    <van-popup v-model:show="showExportFilePopup"
               :style="{'width':'90%','height':'200px'}">
      <div>请自行复制数据并保存为txt格式的文件
      </div>
      <van-button round
                  type="primary"
                  size="normal"
                  @click="copyExportData">复制</van-button>
      <hr>
      <div id="copy-txt">{{exportData}}</div>
    </van-popup>
...
@Options({
  ...
  methods: {
      ...
      axios.get("./data/datetimeType.json").then((res) => {
        ...
        for (let key in localStorage) {
          ...
          if (planDetailContentKey.length > 1) {
            planDetailContent += key + localStorage.getItem(key) + "-r-n";
          }
        }
        this.exportData = "start:-r-n" + planType + plan + planDetailContent;
        this.showExportFilePopup = true;
      });
    },
    importPlanData() {
    ...
    afterReadFile(file: any) {
      let txtContent = Base64.decode(file.content);
      let content = txtContent.split("start:-r-n");
      if (content.length > 1) {
        let dataList = content[1].split("-r-n");
        for (let index in dataList) {
          alert(dataList[index].split('{"'));
          alert(dataKeyValue);
          let dataKeyValue = dataList[index].split('{"');
          localStorage.setItem(dataKeyValue[0], '{"' + dataKeyValue[1]);
        }
    ...
    copyExportData() {
      ...
      input.setAttribute("value", this.exportData);
      input.select();
