<template>
  <div>
    ...
    <van-grid :column-num="2"
              ...
              square
              clickable>
      <van-grid-item v-for="item in planContentList"
                     :key="item.planType"
                     :text="item.planType"
                     @click="planContent(value)" />
    </van-grid>
    {{date}}
  </div>
</template>
  ...
  methods: {
    ...
    getCalendarComponentDate(date: Date) {
      ...
      while (isNext) {
        ...
        if (item) {
          this.planContentList[index] = JSON.parse(JSON.stringify(item));
          console.log(this.planContentList[index]);
          index++;
          continue;
        }
        break;
